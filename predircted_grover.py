...
st.markdown('Nous avons conçu l\'application afin de permettre à ses utilisateurs de justifier ou de mesurer des anomalies dans des textes suspects. \nNous appelons "anomalies" des caractèristiques propores à une génération par une intelligence artificielle ou par des LLMs, et qui caractèrisent un textes générés plutôt qu\'un texte écrit par un étudiant, c\'est à dire un très grand vocabulaire, de très bons taux lexicaux et verbaux, en comparaison avec des copies \'similaires\' dans leur exercice de conception.')

st.subheader('Comment tout cela fonctionne ?') 

st.markdown("Pour permettre une meilleur analyse, vous veuillerez à utiliser un ensemble de textes identiques (des copies similaires dans l'exercice) afin de comparer (nous développons des solutions totalement automatisées pour ces cas-si), plusieurs copies entre elles. De ce cas ci, les résultats seront plus performants et de grandes qualités pour permettre une étude plus informative, sur les statistiques afin de déceler par l'anomalie des textes générés.De sorte qu\'une copie obtenant des scores incohérents sera décelée comme suspecte et pourra être considérée comme générée, si une étude approfondie décèle des stastiques trop aléatoire.")
